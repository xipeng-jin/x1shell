...
projectionSnapshotLayer("ProjectionSnapshotQuery", (it) => {
  ...
  it.effect("reads thread detail content and snapshot sequence from one projection snapshot", () =>
    Effect.gen(function* () {
      const snapshotQuery = yield* ProjectionSnapshotQuery;
      const sql = yield* SqlClient.SqlClient;
      ...
      if (threadSnapshot._tag === "Some") {
        assert.equal(threadSnapshot.value.snapshotSequence, 7);
        assert.equal(threadSnapshot.value.thread.activities[0]?.sequence, 7);
        assert.equal(threadSnapshot.value.thread.activities[0]?.summary, "sequence seven");
      }
    }),
  );

  it.effect("includes the thread-turns projector in thread detail snapshot sequence", () =>
    Effect.gen(function* () {
      const snapshotQuery = yield* ProjectionSnapshotQuery;
      const sql = yield* SqlClient.SqlClient;
      ...
      yield* sql`
        ...
        VALUES (
          'project-turn-sequence',
          'Project Turn Sequence',
          '/tmp/project-turn-sequence',
          '{"provider":"codex","model":"gpt-5-codex"}',
          '[]',
          '2026-04-03T00:00:00.000Z',
          '2026-04-03T00:00:01.000Z',
      ...
      for (const projector of Object.values(ORCHESTRATION_PROJECTOR_NAMES)) {
        yield* sql`
          ...
          VALUES (
            ${projector},
            ${projector === ORCHESTRATION_PROJECTOR_NAMES.threadTurns ? 6 : 9},
            '2026-04-03T00:00:05.000Z'
          )
        `;
      }
      ...
      const threadSnapshot = yield* snapshotQuery.getThreadDetailSnapshotById(
        ThreadId.make("thread-turn-sequence"),
      );

      assert.equal(threadSnapshot._tag, "Some");
      if (threadSnapshot._tag === "Some") {
        assert.equal(threadSnapshot.value.snapshotSequence, 6);
        assert.equal(
          threadSnapshot.value.thread.latestTurn?.turnId,
          TurnId.make("turn-sequence-7"),
        );
      }
    }),
  );
