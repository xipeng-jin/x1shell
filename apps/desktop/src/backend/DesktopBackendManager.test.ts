  ...
  it.effect("terminates and restarts a backend that never becomes ready", () =>
    Effect.gen(function* () {
      ...
      const spawnerLayer = Layer.succeed(
        ...
        ChildProcessSpawner.make(() =>
          Effect.gen(function* () {
            ...
            const close = Queue.offer(closed, runNumber).pipe(
      ...
      yield* Effect.gen(function* () {
        ...
        const timeoutSnapshot = yield* manager.snapshot;
        assert.equal(timeoutSnapshot.desiredRunning, true);
        assert.equal(timeoutSnapshot.ready, false);
        assert.equal(Option.isNone(timeoutSnapshot.activePid), true);
        assert.isFalse(yield* Ref.get(backendReady));
        assert.equal(readyCount, 0);
