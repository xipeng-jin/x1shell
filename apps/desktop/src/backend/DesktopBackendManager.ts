...
const runBackendProcess = Effect.fn("runBackendProcess")(function* (
  ...
  const exit = Effect.result(handle.exitCode).pipe(Effect.map(describeProcessExit));
  ...
  const earlyExit = yield* Effect.raceFirst(
    readiness.pipe(Effect.as(Option.none())),
    exit.pipe(Effect.map(Option.some)),
  );

  if (Option.isSome(earlyExit)) {
